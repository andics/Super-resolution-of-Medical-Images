function U = SIM ( F, rho, theta, sigma, Sph_n )
...
R1 = sqrt((sigma)*Dim);
R1
[F1, R] = Get_Sphere_1S(F, sigma);
Rs(1) = R;
Fs(1,:,:) = F1;
[InterS(1,:,:), InterR(1), B] = Get_Intersection(F,R1,[],reshape(Fs(1,:,:),m,n),Rs(1));
for i=2:Sph_n
    [FSC, RSC] = Get_Sphere_1S(reshape(Fs(i-1,:,:),m,n), sigma);
    Fs(i,:,:) = FSC;
    Rs(i,:,:) = RSC;
    [InterS(i,:,:), InterR(i), B] = Get_Intersection(reshape(Fs(i,:,:),m,n), Rs(i), B, reshape(InterS(i-1,:,:),m,n), Rs(i-1));
end
S = InterS(Sph_n);
R = InterR(Sph_n);
R
...
%U = F1 + vec*(R1+R2);
%return;
%[S, R, B] = Get_Intersection_1S(F, R1, F1, R2);
%U = S;
%return
%I_sect = SIM(F, sigma);
